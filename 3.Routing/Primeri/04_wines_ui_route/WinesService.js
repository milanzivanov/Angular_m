(function(){
	...
	var winesService = function($http, $log){
		...
		var get = function(id){
			return $http.get(url+"/"+id).then(onSuccess);
		}
		...
		var put = function(data){			
			// API servera je opisan na predavanjima iz AJAX-a. Za ovu metodu pogledati slajd 17 "snimanje podataka": dodavanje novog objekta u kolekciju wines se vrši slanjem POST zahteva na https://localhost:3000/api/wines pri čemu se kao parametar POST zahteva šalje JSON objekat koji će biti dodat u kolekciju 	
			$log.info("Sending http PUT request to " + url+"/"+data._id);
			$log.info("PUT param: " + data)
			return $http.put(url+"/"+data._id, data).then(onSuccess); // $http.post vraća promise objekat nad kojim se može pozvati then metoda. Sama then metoda takođe ima promise objekat kao povratnu vrednost => povratna vrednost post funkcije je promise objekat
		} 
